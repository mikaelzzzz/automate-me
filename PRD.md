...
| 8 executable recipes tempt scope creep | Advised recipes are pure data; every executable recipe is mapped to a timeline day (§5.2); demo commits to hero set (dishwasher, school note or boletos, briefing); cut order if slipping: Guardian check-in (degrades to signals-only) → commute audit → batching. The shared report page is load-bearing for F8 **and** F10's Progress Report — one build serves both; cut last |
| Days 11–12 carry four workstreams before demo recording | End-of-day-12 gate: if the Guardian isn't demo-ready, the recording uses the degraded closing segment defined in §8; report page built once for F8+F10; polish is the first thing dropped |
